...
// GodparentsFormListComponent de Batismo/Primeira Comunhão — quem usa
// (FormComponent) decide quando sincronizar com o backend, no submit(),
// comparando com os padrinhos originais carregados no load().
//
// Modelo híbrido (back: ConfirmationGodparentEntity) — godparent_id
...
// payload de criação (ver FormComponent.syncGodparents).
export interface ConfirmationGodparentRow {
...
@Component({
  selector: 'app-godparents-form-list-confirmations',
...
export class GodparentsFormListComponent implements OnChanges {
  @Input() data: ConfirmationGodparentRow[] = [];
  @Input() companyId: string | null = null;
  @Output() dataChange = new EventEmitter<ConfirmationGodparentRow[]>();
  ...
  constructor(
    ...
    private readonly confirmationGodparentService: ConfirmationGodparentService,
  ) {}

  ngOnChanges(changes: SimpleChanges) {
    if (changes['companyId'] && this.companyId) {
      this.confirmationGodparentService.suggestions('course_place', this.companyId).subscribe({
  ...
  confirmCustomer() {
    ...
    // Mesma regra do backend (ConfirmationGodparentService.create): o mesmo
    // paroquiano não pode ser padrinho duas vezes da mesma crisma — checado
    ...
    const row: ConfirmationGodparentRow = {
      is_external: false,
  ...
  saveEdit(index: number) {
    ...
    const updated: ConfirmationGodparentRow = {
      ...original,
      id: undefined,
