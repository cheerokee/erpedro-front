...
// criação do registro (antes de existir marriage_id), não só na edição.
//
// "Editar" um documento já existente (com id) NÃO pode ser remover +
// readicionar (diferente de testemunha): o arquivo não fica em memória
// depois de enviado, só existe no S3. Por isso só o título é editável pra
// linhas existentes.
export interface MarriageAttachmentRow {
...
@Component({
  selector: 'app-marriage-attachments-form-list',
  templateUrl: './marriage-attachments-form-list.component.html',
  styleUrls: ['./marriage-attachments-form-list.component.scss'],
  imports: [SharedModule, FormsModule, ModalComponent, ModalHeader, ModalBody, ModalFooter],
})
export class MarriageAttachmentsFormListComponent {
  @Input() data: MarriageAttachmentRow[] = [];
  @Output() dataChange = new EventEmitter<MarriageAttachmentRow[]>();
  ...
  constructor(
    private readonly service: MarriageAttachmentService,
  ...
  add() {
    ...
    const row: MarriageAttachmentRow = {
  ...
  download(row: MarriageAttachmentRow) {
