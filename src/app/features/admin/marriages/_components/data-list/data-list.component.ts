...
import {
  ...
  MarriageListResult,
  MarriageService,
} from '../../../../../@core/modules/parishioner/services/marriage.service';
import { ModalMarriagesComponent } from '../modal/modal.component';
...
@Component({
  selector: 'app-data-list-marriages',
  ...
  imports: [
    ...
    NgbDropdownModule,
    FilterComponent,
    ModalMarriagesComponent,
  ],
  providers: [FilterStore],
})
export class DataListComponent implements OnInit, OnDestroy {
  rows: MarriageListItem[] = [];
  meta = { totalItems: 0, itemCount: 0, itemsPerPage: PAGE_SIZE };
  page = 1;
  loading = false;

  @ViewChild('modal') modal: ModalMarriagesComponent;

  private readonly destroy$ = new Subject<void>();
  private filter = new MarriageModel.Filter({});
    ...
    this.filterStore.store$
      .pipe(
        switchMap((filter) => {
          ...
          this.page = 1;
          this.loading = true;
          return this.marriageService.list(this.filter, this.page, PAGE_SIZE);
        }),
  ...
  private applyResult(result: MarriageListResult) {
    this.loading = false;
